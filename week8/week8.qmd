...
# plot 
plotRGB(landsat, r = 3, g = 2, b = 1, stretch = "lin") # stretch raster linearly (0-255)
...
#%>%
SB_county_south <-  SB_county_south %>% st_transform(crs = crs(landsat))
```
...
```{r include=TRUE}
# crop landsat scene to extent of SB county
landsat_crop <- crop(landsat, SB_county_south) 

# mask raster to southern portion of SB county 
# some values still exist as NA, so need to get rid of 
landsat_mask <- mask(landsat_crop, SB_county_south)

# remove unneccasry objects from environment
rm(landsat, SB_county_south, landsat_crop) # no longer need

# plot to ensure mask has dropped na values
...
# Reclassify for potentially erroneous values as NA
rcl <- matrix(c(-Inf, 7272, NA,
                43637, Inf, NA), ncol = 3, byrow = T)

#reclassify 
landsat <- classify(landsat_mask, rcl = rcl)

# adjust values by scaling factor
landsat <- (landsat*.000275-.2) *100

# check that valyes are 0-100
summary(landsat)
...
tm_shape(training_data) +tm_polygons(fill ="type")
```
...
```{r}
## Find the cells within all type polygons that we have data for

# extract pixel values from landsat raster at the locations in the training data 
training_data_values <- terra::extract(landsat, training_data, df = T) 

# convert training data to data frame (no geom)
training_data_attributes <- training_data %>% st_drop_geometry()

# join training data attributes (description of classification) and training data extracted values
sb_training_data <- left_join(training_data_values, training_data_attributes, 
                              by = c("ID"="id")) %>% mutate(type = as.factor(type)) # convert landcover type to factor

# double check type
str(sb_training_data)
...
```{r include=TRUE}
# now have the training data... classifier (decision tree) will use this 
# establish formula
sb_formula <- type ~ red+ green+ blue + NIR + SWIR1 + SWIR2

# make classifier
sb_decisiontree <- rpart(formula = type ~ red+ green+ blue + NIR + SWIR1 + SWIR2, data = sb_training_data, 
...
```{r}
## what would landsat look like if it uses every pixel from tree?

# use predict()
sb_classification_dt <- terra::predict(landsat, sb_decisiontree, 
                                   type = "class", na.rm = T)


# inspect levels of training data to understand order of classes
...
```{r}
# plot results
dt_plot <- tm_shape(sb_classification_dt) + tm_raster(
  col.scale = tm_scale(
...
# now for every cell in raster, we have information on predicted values per type 
# look at training data, so look at first observation then classifying baaed on percentages in blue, green, red, etc. columns
...
```{r}
# # convert raster to data frame and scale it 
# landsat_df <- as.data.frame(landsat, xy = T)
# 
# # extract spectral bands for clustering
# spectral_data <- landsat_df[,c("blue", "green", "red", "NIR", "SWIR1", "SWIR2")]
# #is.na(spectral_data)

# distance based algorithm (euclidian distance from point to center) 
# must scale with different variables that have different units
# scale to make sure distance part is not affected
spectral_scale <- scale(spectral_data) # we only have cells that have their reflectance value
...
# perform k means = 4 (classify with 4 group "types")
...
values(SB_classification_kmeans)[cells] <- landsat_df$cluster # only look at cells that have lat and long (x and y)
...
```{r}
landsat_df %>% group_by(cluster) %>% 
  summarize(blue = mean(blue), 
  green = mean(green), 
  red = mean(red), 
  NIR = mean(NIR), 
  SWIR1 = mean(SWIR), 
  SWIR2 = mean(SWIR2)) %>% arrange(cluster) %>% 
