...
```{r}
# create a data frame for attributes (associated values)
...
```{r}
world_df <- world %>% 
  select(-geom) # does not deselect column
...
```{r}
display_crs <- tm_shape(world, crs = 8857) + tm_fill(fill = "area_km2")
display_crs
...
transformedcrs <- tm_shape(world_transformed) + tm_fill(fill = "area_km2") 

tmap_arrange(display_crs, transformedcrs)
...
```{r}
world %>% select(-subregion, -area_km2)
```
...
```{r}
world2 <- world %>% filter(lifeExp >= 80)


world %>% filter(continent == "Asia") %>% 
  select(name_long, continent, lifeExp) %>% slice_max(lifeExp) %>% 
  st_drop_geometry()
...
world %>% group_by(continent) %>% 
  summarize(population = sum(pop, na.rm = T), 
            area_km2 = sum(area_km2, na.rm = T), 
            n_countries = n(),
            #population_den = (pop/area_km2), 
            pop_density = round(sum(population/area_km2),3)) %>% arrange(n_countries) %>%
  slice_max(n = 3) %>% 
            # highest_pop = max(population_den), 
            # n_countries = n())
st_drop_geometry()
...
```{r}
world_coffee_inner <- inner_join(world, coffee_data, by  = "name_long") # make sure not losing any data

if (nrow(world_coffee_nner != nrow(coffee_data))  
