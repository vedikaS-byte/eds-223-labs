...
```{r}
# different format... must plot (no physical attribute)
class(zion_elevation)

dim(zion_elevation) ## 1 layer is last # (elevation)
...
# find out # of layers in a raster object
nlyr(landsat)
...
```{r}
#landsat34 <- subset(landsat, c("landsat_3","landsat_4"))
...
```{r}
# Plot
map1 <- tm_shape(austria) + tm_raster(col.legend = tm_legend("Elevation(m)")) + 
  tm_title(text = "Austria")


map2 <- tm_shape(switzerland) + tm_raster(col.legend = tm_legend("Elevation(m)")) + 
  tm_title(text = "Switzerland")

map3 <- tm_shape(merged) + tm_raster(col.legend = tm_legend("Elevation(m)")) + 
...
```{r}
global(zion_elevation, sd) # no sample, using whole raster
st_crs(zion_elevation)
```
...
```{r}
landsat[1] <- cbind(c(0),c(0), c(0), c(0)) # as many rows
landsat[1] # recall it is a matrix
```
...
# Two ways to extract: Find cell ID for point 
...
# Approach 2: Extract raster value at point
...
```{r}
# Now a spatial raster
zion_elevation_clip <- zion_elevation[clip,drop = F]
```

```{r}
tm_shape(zion_elevation) + tm_raster(col.legend = tm_legend(title = "Elevation (m)")) + 
...
# Set all cells that have < 2000 m elevation to NA
rmask[rmask < 2000] <- NA

# If wanted to access with multiple layers, then need to specify the layer in []
   
# Subset elevation raster based on mask 
# Approach 1: Bracket subsetting 
mask1 <- zion_elevation[rmask,drop = F]
# Approach 2: Using mask function
mask2 <- mask(zion_elevation, rmask)
```

```{r}
# Now make some maps with masks 
map1 <- tm_shape(zion_elevation) + tm_raster(col.legend = tm_legend(title = "Elevation (m)")) + tm_title(text = "Original")

map2 <- tm_shape(mask1) + tm_raster(col.legend = tm_legend(title = "Elevation (m)")) + tm_title(text = "Bracket subsetting")

map3 <- tm_shape(mask2) + tm_raster(col.legend = tm_legend(title = "Elevation (m)")) + tm_title(text = "mask()")
...
zion_elevation >5 # determines whether each cell has value greater than 5
...
summary(reclassified) # Will give # of cells within each group

```

```{r}
# Make a map to look at orginal map vs reclassified map
map1 <- tm_shape(zion_elevation) + tm_raster(col.legend = tm_legend(title = "Elevation (m)")) + tm_title("Original")

map2 <- tm_shape(reclassified) + tm_raster(col.legend = tm_legend(title = "Elevation (m)")) + tm_title("Reclassified")
...
```{r}
# Function to calculate ndvi for rasters
ndvi_fun <- function(nir, red){
  (nir-red)/(nir+red)
}

# Apply function to landsat bands 3 and 4 
ndvi_raster <- lapp(landsat[[c(4,3)]], fun = ndvi_fun) # Need same order as function, so near infrared first
...
```{r}
elevation_focal <- focal(zion_elevation, # pass data
                         w = matrix(1,nrow = 9, ncol = 9), # Look at all cells equally- first argument is the value to be stored in cells
                         fun = min
                         )
...
```{r}
map1 <- tm_shape(zion_elevation) +
  tm_raster(col.legend = tm_legend(title = "Elevation (m)")) + tm_title("Original")

map2 <- tm_shape(elevation_focal) +
  tm_raster(col.legend = tm_legend(title = "Elevation (m)")) + tm_title("Aggregate with focal")
...
zonal(zion_elevation, reclassified, fun = "mean") %>%
  ...
  #kable_styling(bootstrop_options = "striped")
...
```{r}
elev_2 <- extend(zion_elevation, c(1,200)) # Manually add new cells
```

Performing algebraic operations on objects with different extents doesn't work.

```{r}
zion_elevation + elev_2 # Throws error
...
elev_3 + elev_2 # Now works since extents match; this is a local operation
...
```{r}
# Get the resolutions to match by matching extents
# CHANGING SHAPE OF RASTER vs other times where cells are still there but NA
# Can do this with aggregation (coarsen)
...
map2 <- tm_shape(zion_elevation_coarse) + tm_raster(col.legend = tm_legend(title = "Elevation (m)")) + tm_title("Coarse (aggregated)")
...
```{r}
# Disaggregate aggregated raster
...
map3 <- tm_shape(zion_elevation_disagg) + tm_raster(col.legend = tm_legend(title = "Elevation (m)")) + tm_title("Finer (disaggregated)")
